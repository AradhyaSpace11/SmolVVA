...
# --- 2. THREADED INPUTS (CONTROLLER & SERIAL) ---
controller_state = {
    "LX": 0, "LY": 0, "RX": 0, "RY": 0, 
    "LT": 0, "RT": 0, "BTN_B": 0
}

# Shared variable for the Serial Yaw
serial_yaw_deg = 0.0
...
def serial_listener():
    global serial_yaw_deg
    try:
        # Update port name if necessary
        ser = serial.Serial('/dev/ttyUSB0', 115200, timeout=1)
        while True:
            line = ser.readline().decode(errors="ignore").strip()
            if line:
                try:
                    # Extracts numerical value and updates global state
                    serial_yaw_deg = float(line)
                except ValueError:
                    pass
...
# --- 4. MAIN LOOP ---
print("System Active! Serial controls Yaw, Controller controls Arm/Gripper.")

while True:
    # 1. Process Inputs
    
    # YAW CONTROL: Overwrite pos[0] with Serial data (converted to Radians)
    pos[0] = math.radians(serial_yaw_deg)

    # OTHER CONTROLS: Standard Xbox inputs
    move_speed = 0.03
